...
if __name__ == "__main__":
    ...
    content_list = [x.page_content for x in st.session_state.splitted_result]
    metatdata_list = [x.metadata for x in st.session_state.splitted_result]

    if st.session_state.splitted_result: 
        content_list = [x.page_content for x in st.session_state.splitted_result]
        metatdata_list = [x.metadata for x in st.session_state.splitted_result]
        col3331, col3332 = st.columns(2)
        with col3331: content_list
        with col3332: metatdata_list
        
        
        
        with st.expander("Full Result"): st.session_state.splitted_result
