...
def convert_pdf_to_markdown(path):
    ...
    doc = fitz.open(path)  # open input file
    ...
    out = open(doc.name.replace(".pdf", ".md"), "w", encoding='utf-8')
    out.write(md_string)
    out.close()
...
if "path3" not in st.session_state:
    st.session_state.path3 = ""
...
if __name__ == "__main__":
    ...
    with st.container():
        file_list2 = list_all_files(base_dir)
        sel21 = st.selectbox("📌 파일을 선택해주세요", file_list2, index=None)
        sel21
        if sel21:
            st.session_state.path3 = os.path.join(base_dir, sel21)

        if st.session_state.path3: st.session_state.path3

    with st.spinner("Converting..."):
        if st.button("Converting to MarkDown"):
            # file_name = sel31.split(".")[0]
            convert_pdf_to_markdown(st.session_state.path3)
            st.info("Converting PDF to Markdown is Completed")
